...
export default function AdminOrders() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      // Use different endpoints based on active filter
      const endpoint = activeFilter === 'orders' 
        ? `${API_BASE_URL}/orders/retailer/active-orders`
        : `${API_BASE_URL}/orders/retailer/order-history`;
      ...
      if (data.success) {
        ...
        console.log(`Fetched ${data.orders?.length || 0} orders for ${activeFilter}`);
      } else {
        setOrders([]);
      }
    } catch (error) {
  ...
  // FIX: Add activeFilter as dependency to automatically fetch when tab changes
  useEffect(() => {
  ...
  }, [authToken, authLoading, isAuthenticated, activeFilter]); // Added activeFilter dependency

  // No need for frontend filtering now since backend handles it properly
              ...
              >
                ...
                {isExpanded && (
                  <View style={styles.expandedContent}>
                    ...
                    {/* Progress - Only show for active orders and non-cancelled orders */}
...
//             // Fetch orders immediately when tab is clicked
//             fetchOrders();
//           }}
//         >
//           <Text style={[styles.filterButtonText, activeFilter === 'orders' && styles.filterButtonTextActive]}>
